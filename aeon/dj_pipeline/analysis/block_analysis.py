...
class BlockPatchPlots(dj.Computed):
    ...
    def make(self, key):
        ...
        subj_wheel_cumsum_dist = (BlockSubjectAnalysis.Patch & key).fetch(
            "subject_name", "patch_name", "wheel_cumsum_distance_travelled", as_dict=True
        )
        ...
        for subj in subject_names:
            for patch_name in patch_names:
                ...
                cur_cum_pel_ct = pd.merge_asof(
                    ...
                    pd.DataFrame(
                        {"time": wheel_ts[patch_name], "cum_wheel_dist": cur_cum_wheel_dist}
                    ).sort_values("time"),
                    on="time",
                    direction="forward",
        ...
        for subj in subject_names:
            for patch_name in patch_names:
                ...
                cur_cum_pel_ct = pd.merge_asof(
                    cum_pel_ct[(cum_pel_ct["subject"] == subj) & (cum_pel_ct["patch"] == patch_name)],
                    pd.DataFrame(
                        {"time": wheel_ts[patch_name], "run_wheel_pref": cur_run_wheel_pref}
                    ).sort_values("time"),
                    on="time",
                    direction="forward",
                    tolerance=pd.Timedelta("0.1s"),
        ...
        for subj in subject_names:
            for patch_name in patch_names:
                ...
                cur_cum_pel_ct = pd.merge_asof(
                    cum_pel_ct[(cum_pel_ct["subject"] == subj) & (cum_pel_ct["patch"] == patch_name)],
                    pd.DataFrame(
                        {"time": wheel_ts[patch_name], "run_time_pref": cur_run_time_pref}
                    ).sort_values("time"),
                    on="time",
                    direction="forward",
                    tolerance=pd.Timedelta("0.1s"),
